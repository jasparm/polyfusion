...
export class VertexManager {
    vertexMap;
    selectedVertices; // this stores a list of all the currently selected vertices
    id;
    groupID;
    constructor(groupID) {
    ...
    init(vertices) {
        for (let i = 0; i < vertices.length; i += 3) {
            this.vertexMap.set(this.id, new THREE.Vector3(vertices[i], vertices[i + 1], vertices[i + 2]));
            this.id++;
        }
    }
    /**
